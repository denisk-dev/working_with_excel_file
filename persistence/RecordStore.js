...
function readStudentsData(fileName) {
  ...
  try {
    ...
    for (let k = 1; k < studentsData.length; k++) {
      studentsDataUpdated.push(new studentObject(studentsData[k][0].slice(1, -1), studentsData[k][1].slice(1, -1), studentsData[k][2].slice(1, -1), studentsData[k][3].slice(1, -1), studentsData[k][4].slice(1, -1), studentsData[k][5].slice(1, -1), studentsData[k][6].slice(1, -1), studentsData[k][7].slice(1, -1), studentsData[k][8].slice(1, -1), studentsData[k][9].slice(1, -1), studentsData[k][10].slice(1, -1), studentsData[k][11].slice(1, -1), studentsData[k][12].slice(1, -1), studentsData[k][13].slice(1, -1), studentsData[k][14].slice(1, -1), studentsData[k][15].slice(1, -1), studentsData[k][16].slice(1, -1)))
    }
...
function writeToCSVFile(data) {
  ...
  for (let i = 0; i < data.length - 1; i++) {
    ...
    if(i == data.length-2)
      dataToPersist +='"' + data[i].Decim + '"';
    else
      dataToPersist +='"' + data[i].Decim + '"' + "\n";
  }

  let nameFile = (data[data.length - 1].newFileName == null) ? './13100262-eng/13100262.csv' : './NewCSV/' + data[data.length - 1].newFileName + '.csv';
